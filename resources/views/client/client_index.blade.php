                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            ...
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Clients Listing
                            </h1>
                        ...
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    <!--begin::Add Clients-->
                                    <button type="button" class="btn btn-primary me-2" onclick="addClient()">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($clinet); $i++) @php $a=$i; $a++; @endphp
                                        <tr>
                                            <td>{{$a}}</td>
                                            <td><a href="#" onclick="showClient('{{$clinet[$i]->id}}')">{{ucwords($clinet[$i]->client_name)}}</a></td>
                                            <td>{{$clinet[$i]->client_note}}</td>
                                            <td>{{date('Y-m-d',strtotime($clinet[$i]->created_at))}}</td>
                                            <td>
                                                <button  class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2"  data-bs-original-title="Edit Model" onclick="editClient('{{$clinet[$i]->id}}')">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('client.destroy', $clinet[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
...
<script>
   function showClient(id) {
        url = "{{ route('client.show', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#modalXlNoHeader').html(result);
                $('#myModalXlNoHeader').modal('show');
            }
        });
    }
